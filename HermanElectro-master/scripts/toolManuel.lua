...
local function drawAtTile(image, tileX, tileY, x, y, rotation)
	drawHere(image, x + tileX*tileWidth, y + tileY*tileHeight, rotation)
end

P.sawScreens = {
function(x, y, scale)
	drawAtTile(tiles.wall.sprite, 1, 0, x, y)
	drawAtTile(player.character.sprite, 0, 0, x, y)
	drawAtTile(green, 1, 0, x, y)

	drawAtTile(tiles.wall.destroyedSprite, 8, 0, x, y)
	drawAtTile(player.character.sprite, 7, 0, x, y)
end,
function(x, y, scale)
	drawAtTile(tiles.metalWall.sprite, 1, 0, x, y)
	drawAtTile(player.character.sprite, 0, 0, x, y)
	drawAtTile(green, 1, 0, x, y)
	...
	drawAtTile(tiles.metalWall.poweredSprite, 1, 5, x, y)
	drawAtTile(tiles.wire.poweredSprite, 1, 6, x, y)
	drawAtTile(player.character.sprite, 0, 5, x, y)
	drawAtTile(green, 1, 5, x, y)

	drawAtTile(tiles.powerSupply.sprite, 8, 4, x, y)
	drawAtTile(tiles.metalWall.destroyedSprite, 8, 5, x, y)
	drawAtTile(tiles.wire.sprite, 8, 6, x, y)
